...
class BookingService:
    ...
    async def get_available_rooms(self, min_capacity: int = 0) -> List[Room]:
        return await self.room_repo.list_available(min_capacity)

    async def book_room(self, room_id: int, user_id: int, start_time: datetime, end_time: datetime, people_count: int) -> Booking:
