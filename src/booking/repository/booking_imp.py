...
class BookingRepository(IBookingRepository):
    ...
    async def get(self, id: int) -> Optional[BookingOut]:
        query = select(bookings).where(bookings.c.id == id)
        result = await self.session.execute(query)
        row = result.first()
        if row:
            return BookingOut(**row._mapping)
        return None

    async def list(self, filters: Dict[str, Any] = None) -> List[BookingOut]:
        query = select(bookings)
        if filters:
            for key, val in filters.items():
                query = query.where(getattr(bookings.c, key) == val)
        result = await self.session.execute(query)
        return [BookingOut(**row._mapping) for row in result.fetchall()]
    ...
    async def add(self, entity: BookingOut) -> BookingOut:
        self.session.add(entity)
        await self.session.commit()
        return entity

    async def update(self, entity: BookingOut) -> None:
        pass
