...
class RoomRepository(IRoomRepository):
    ...
    async def get(self, id: int) -> Optional[RoomOut]:
        query = select(rooms).where(rooms.c.id == id)
        result = await self.session.execute(query)
        row = result.first()
        if row:
            return RoomOut(**row._mapping)
        return None

    async def list(self, filters: Dict[str, Any] = None) -> List[RoomOut]:
        query = select(rooms)
        if filters:
            for key, val in filters.items():
                query = query.where(getattr(rooms.c, key) == val)
        result = await self.session.execute(query)
        return [RoomOut(**row._mapping) for row in result.fetchall()]
    ...
    async def add(self, entity: RoomOut) -> RoomOut:
        pass

    async def update(self, entity: RoomOut) -> None:
        pass
